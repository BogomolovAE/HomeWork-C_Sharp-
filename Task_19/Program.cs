...
if (number==0||number/10==0) Console.WriteLine("Number is the polynomial");
else Console.WriteLine("Number is not the polynomial"); */
 //                              VARIANT 2 (WHITH STRING)
 Console.WriteLine("Enter a number: ");
string numberInString=Check(Console.ReadLine());
string Check (string N)                                         // Процедура проверки значений введенной строки
{
    m1:
    while (N=="")
...
for (int i = 0; i<=numberInString.Length/2-1; i++)
{
    if (numberInString[i]!=numberInString[numberInString.Length-i-1])
    
    {
        Console.WriteLine("Number is not the polynomial");
        break;
    }
    if (i==numberInString.Length/2-1) Console.WriteLine("Number is the polynomial");
}